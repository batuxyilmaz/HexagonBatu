...
public class InputManager : SuperClass {
	...
	/* İlk basılma var mı yok mu kontrol ediyor */
	private void TouchDetection() 
	{
		/* başlangıç pozisyonunu ayarlıyor ilk basılma için */
	...
	private void CheckSelection(Collider2D collider) {
		...
		if (collider != null && collider.transform.tag == TAG_HEXAGON)
		{
			...
			if (Input.GetTouch(ZERO).phase == TouchPhase.Ended && IsTouched)
			{
				IsTouched = false;
	...
	/* herhangi bir rotasyon durumu var mı yok mu kontrol ediyor */
	private void CheckRotation() {
		if (Input.GetTouch(ZERO).phase == TouchPhase.Moved && IsTouched) 
		{
			...
			/* Eğer rotasyon varsa ilk basılma poziyonu ile o an ki basılan pozisyonun arasındaki mesafeyi kotnrol ediyor */
			if ((Mathf.Abs(distanceX) > HEX_ROTATE_SLIDE_DISTANCE || Mathf.Abs(distanceY) > HEX_ROTATE_SLIDE_DISTANCE) && selectedHexagon != null)
			{
				...
				bool triggerX = Mathf.Abs(distanceX) > Mathf.Abs(distanceY); //eğer rotasyon varsa onu ayarlıyor
				bool swipeRightUp = triggerX ? distanceX > ZERO : distanceY > ZERO; //eğer swipe yönü sol ve ya sağ ise
				bool touchBiggerThanHex = triggerX ? touchCurrentPosition.y > screenPosition.y : touchCurrentPosition.x > screenPosition.x; //eğer basılma pozisyonunun değeri hexagon pozisyonundan büyükse
				bool clockWise = triggerX ? swipeRightUp == touchBiggerThanHex : swipeRightUp != touchBiggerThanHex; //eğer x eksenninde rotasyon oluyorsa ve swipe işlemi aynı yöndeyse saat yönünde değilse tersine döndür
